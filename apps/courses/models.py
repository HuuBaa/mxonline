...
class Course(models.Model):
    ...
    you_need_know=models.CharField(default="",verbose_name=u"课程需知",max_length=300)
...
class Lesson(models.Model):
    ...
    def __unicode__(self):
        return "章节：{0}({1})".format(self.name,self.course)
...
class Video(models.Model):
    ...
    def __unicode__(self):
        return "视频：{0}({1})".format(self.name, self.lesson)
...
class CourseResource(models.Model):
    ...
    def __unicode__(self):
        return "课程资源：{0}({1})".format(self.name, self.course)